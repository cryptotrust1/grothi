...
async function main() {
  ...
  // Create admin user (info@grothi.com)
  const adminPassword = await hash('Vladimir54793347@', 12);
  const admin = await prisma.user.upsert({
    where: { email: 'info@grothi.com' },
    update: {
  ...
  console.log('Admin user: info@grothi.com');
}
